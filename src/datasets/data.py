...
class ASDData(BaseData):
    """
    Dataset class for ASD dataset.
    Attributes:
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        self.all_df, self.data_len = self.load_all(root_dir, file_list=file_list, pattern=pattern)
        self.all_IDs = list(self.all_df.index.unique())  # all sample (session) IDs
        ...
        for i in range(len(self.data_len)):
            del_point += self.data_len[i]
            for j in range(1,self.max_seq_len):
                self.all_IDs.remove(del_point-j)


        self.feature_names = [column for column in self.all_df]
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        if pattern is None:
            ...
            selected_paths = list(filter(lambda x: re.search(pattern.lower(), x), data_paths))
        ...
        #     logger.info("Loading {} datasets files using {} parallel processes ...".format(len(input_paths), _n_proc))
        #     with Pool(processes=_n_proc) as pool:
        #         all_df = pd.concat(pool.map(ASDData.load_single, input_paths))
        # else:  # read 1 file at a time
        #     all_df = pd.concat(ASDData.load_single(path) for path in input_paths)
        data_len = []
        for path in input_paths:
            data_len.append(ASDData.read_data_len(path))

        all_df = pd.concat(ASDData.load_single(path) for path in input_paths)
        all_df = pd.DataFrame(np.array(all_df))
        return all_df, data_len

    @staticmethod
    def load_single(filepath):
        df = ASDData.read_data(filepath)
        # df = ASDData.select_columns(df)
